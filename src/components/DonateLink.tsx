...
const DonateLink: React.FC<{ image: string; name: string; kofiName: string }> = ({ image, name, kofiName }) => {
  return (
    <div className="flex w-32 flex-col items-center justify-center gap-2 font-medium">
      <span>{name}</span>
      ...
      <a href={`https://ko-fi.com/${kofiName}`} target="_blank" className="w-full">
        <button className="flex w-full items-center justify-center gap-2 rounded-md bg-sky-500/20 p-2 text-sky-500 hover:bg-sky-500/30 border-0">
          <IconCoffee />
          Support
        </button>
      </a>
